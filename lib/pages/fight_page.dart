...
const pref_stats_won = 'stats_won';
const pref_stats_lost = 'stats_lost';
const pref_stats_draw = 'stats_draw';
...
class FightPageState extends State<FightPage> {
  ...
  void processGo() {
    setState(() {
      ...
      if (_isGoEnabled) {
        ...
        actionResult = checkResult();

        FightResult? result = FightResult.calculateResults(yourLives, enemysLives);
        if (result != null) {
          print("Saving value"); //TODO extract to method
          SharedPreferences.getInstance().then((prefs) {
            prefs.setString("last_fight_result", result.result);
            switch (result) {
              case FightResult.won:
                {
                  final current = prefs.getInt(pref_stats_won) ?? 0;
                  prefs.setInt(pref_stats_won, current + 1);
                  break;
                }
              case FightResult.lost:
                {
                  final current = prefs.getInt(pref_stats_lost) ?? 0;
                  prefs.setInt(pref_stats_lost, current + 1);
                  break;
                }
              case FightResult.draw:
                {
                  final current = prefs.getInt(pref_stats_draw) ?? 0;
                  prefs.setInt(pref_stats_draw, current + 1);
                  break;
                }
            }
          });
  ...
  String checkResult() {
    if (enemysLives == 0 && yourLives != 0) {
      return ActionResult.won.name;
    }
    if (enemysLives == 0 && yourLives == 0) {
      return ActionResult.draw.name;
    }
    if (enemysLives != 0 && yourLives == 0) {
      return ActionResult.lost.name;
    }
      ...
      child: SizedBox(
        ...
        child: DecoratedBox(
          decoration: BoxDecoration(
            ...
            border: selected ? null : Border.all(color: FightClubColors.darkGreyText, width: 2),
          ),
          child: Center(
            child: Text(
